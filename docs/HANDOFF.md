...
### 9.13 音声認識の途中停止と再開の堅牢化（対応済み・実機挙動は未確認）

ユーザー報告: 認識が途中で止まってしまうことがある。

原因の整理: ChromeのWeb Speech APIは無音・サービス再起動・ページ非表示などでセッションを頻繁に終了/中断し、`onend`や`aborted`が頻発する。従来実装はこの再開経路が脆かった。

1. **同一インスタンスの再利用** — `SpeechRecognition`オブジェクトを生成したまま全再開で使い回していたため、エラー/終了を繰り返すとChrome側の内部状態が固まり得る。
2. **再試行間隔の伸び** — `start()`自体が例外を投げるケースでも`onend`用の指数バックオフ（最大5秒）を使っていたため、認識が数秒単位で間延びした。
3. **マイクトラックの終了** — デバイス切断・OS側の切替で`MediaStreamTrack`が`ended`になると、`start(track)`が永久に失敗し続けた。
4. **無言の停止** — `onerror`も`onend`も来ないままセッションだけ死ぬケースを検知できなかった。

対応（`src/speech/speech-recognizer.js`、自動テスト11件に拡張）:

- セッション終了（`onend`）ごとに認識インスタンスを作り直す（`#createRecognition()`）。再開は常にクリーンなオブジェクトから始まる。
- 再開前にマイクトラックの`readyState`を確認し、`ended`なら`getUserMedia`を再取得する（`#ensureUsableStream()`）。exact指定のデバイスが消えていた場合は既定デバイスへフォールバック（`OverconstrainedError`時）。
- `start()`が例外を投げた場合は専用の短い再試行（500→1000→2000msで打ち切り）に分離。`onend`経由のバックオフ（最大5秒）はセッションが即死を繰り返す場合にだけ効く。
- `aborted`（ページ非表示・サービスリセットが主因）はエラー表示せず「再開待ち」として静かに再開。
- ウォッチドッグを追加。`silenceLimitMs`（既定90秒、コンストラクタオプションのみ）を超えて結果が来ない場合、セッションが無言で停止したとみなして強制再開する（5秒間隔で監視）。
- `stop()`と再開処理の競合ガード。再開のマイク再取得中に停止が入った場合は新しいセッションを開始しない。

未確認・注意点（Phase 0で記録する項目）:

- `silenceLimitMs`の既定90秒は暫定値。長い沈黙（配信者の無言時間）と「無言で死んだセッション」の切り分けは実機で確認する。沈黙中も90秒ごとに1回の再開が入る（無害だがログに「再開待ち」が出る可能性あり）。
